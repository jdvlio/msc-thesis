...
\end{thm}
\begin{proof}
	We proceed by induction on $k$. If $k=1$ then there must exist an infinite
	...
	Without loss of generality we may assume $0\notin \bigcup C_i$ iff $n<i<m$.
	Now define
	\begin{equation}
		B_i=\setbuild{s^{-1}[X]}{X\in\subsets{k}{\posnats}\text{ and
		}X\cup\set{0}\in C_i},
	\end{equation}
	for each $i=0,\dotsc,n$.  Since $\mathcal{C}$ is a partition of
	$\subsets{k+1}{\nats}$, $s(x)\neq 0$ for any $x\in\nats$ and $s^{-1}$ is
	bijective it follows that $\mathcal{B}=\set{B_0,\dotsc,B_n}$ is a
	...
	assume $\subsets{k}{H}\subseteq B_0$.  Consequently, it follows by
	definition that $s[H]\subseteq C_0$, as required.
\end{proof}
