...
\begin{dfn}[Condensations]
		...
		that, for every $a,b\in\alpha$, it holds that: \begin{equation} a\sim
		b\iff \pi(a)=\pi(b), \end{equation} then we call $\pi$ the
		\textbf{splitting} of $\alpha$ \textit{induced by} $\sim$, refer to the
		quotient $\faktor{\alpha}{\sim}$ as a \textbf{condensation} of $\alpha$
		and write $\pi\colon\alpha\tto\faktor{\alpha}{\sim}$.
\end{dfn}
