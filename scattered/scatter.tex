...
\end{thm}
\begin{proof}
	Suppose $\alpha\models\Th(\scattered)$, where $\alpha$ is countable, and
	fix any $n\in\nats$.  Now define a binary relation $R$ on $\domain{\alpha}$
	such that $aRb$ iff there exists a $\beta\in\scattered$ such that
	$(a,b)\nequiv{n}\beta$.
	...
	\begin{proof}[Proof of claim]
		Choose any $\gamma\in\faktor{\alpha}{\sim}$.  If there exists a least
		element $a_0\in\gamma$ then, since $\gamma$ is countable, there exists a
		cofinal sequence $\family{a_i}{i<\omega}$.

		By Ramsey's Theorem, if $h$ is the colouring of $\alpha$ that sends each
		interval $(a,b)$ to its its characterstic in $\chi$, then there exists a
		...
		\end{equation}
		where all of the summands are $n$-equivalent to the same $\one+\beta$,
		...
		greatest element by dualising the argument above (in other words,
		substitute $>$ for each occurence of $<$).
	...
	\begin{proof}[Proof of claim]
		...
		relation $\epsilon_0<\epsilon_1$ and assume, by way of contradiction,
