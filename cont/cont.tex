...
\begin{prp}
	Suppose $\alpha$ is a normal $k$-coloured linear order of order type $\omega$ and $\pi$ is any $k$-pattern.  There then exists a sentence $\sigma_\pi$ such that $\alpha\models\sigma_\pi$ iff $\pi$ cannot be embedded into $\alpha$ as an initial segment.
\end{prp}
\begin{proof}
	Suppose our language contains the colours $r_i$, for each $i<k$, and the $k$-pattern $\pi$ is ordered as: $a_0<a_1<\dotsb<a_{n-1}$.  We may also assume, for each $j<n$, that $r^\alpha_j(a_j)$.

	Let us now define the sentence $\sigma_\pi$ given by:
	\begin{equation}
	\end{equation}
\end{proof}
\begin{proof}
