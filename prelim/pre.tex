...
\begin{dfn}[Regular expansion]
	Suppose $\mathfrak{A}$ is an $L$-structure and, for some $n\in\nats$ and $\bar{a}\in A^n$, let $L^\prime\supseteq L(\bar{a})$.  If the signature $L^\prime$ is obtained from $L(\bar{a})$ by adding finitely many additional relation symbols then an $L^\prime$-structure $\mathfrak{A}^\prime$ is called a \textbf{regular expansion} of $\mathfrak{A}$ whenever $\mathfrak{A}^\prime$ is a definitional expansion of $(\mathfrak{A},\bar{a})$.
\end{dfn}
